...
#define OPENINGDAY_START		(NON_SECURE_MEID_START+NON_SECURE_MEID_LENGTH)
#define OPENINGDAY_LENGTH		(SECTOR_SIZE*SECTOR_SIZE_DEFAULT)	
//<< [FEATURE_SKY_CP_NEW_OPENING_DAY]	

//CONFIG_PANTECH_ERR_CRASH_LOGGING
#define PANTECH_ERR_CRASH_DUMP_START                 (OPENINGDAY_START+OPENINGDAY_LENGTH)
...
// --- ICS items must be added above!!

// --- CAUTION!! below items are added on JB UPGRADE ---

// p13783 : add for Factory Init All command
#define FACTORY_INIT_ALL_PROCESS_START        (PANTECH_SECBOOT_RESULT_START+PANTECH_SECBOOT_RESULT_LENGTH)
...
// 또한 LINUX의 sky_rawdata.h(\modem_proc\core\pantech\sky_rawdata\inc)에도 동일하게 update 해주세요!!!
...
extern int sky_rawdata_get_cksum_rom(char *buf); //goni
extern int sky_rawdata_read_cksum_file(char *buf); 
